<template>
  <Rating unstyled :pt="theme" :pt-options="{ mergeProps: ptViewMerge }">
    <template #onicon="{ toggleCallback }">
      <StarFillIcon
        class="text-base w-4 h-4 transition-colors duration-200 text-primary"
        @click="toggleCallback"
      />
    </template>
    <template #officon="{ toggleCallback }">
      <StarIcon
        class="text-surface-500 dark:text-surface-400 text-base w-4 h-4 transition-colors duration-200"
        @click="toggleCallback"
      />
    </template>
