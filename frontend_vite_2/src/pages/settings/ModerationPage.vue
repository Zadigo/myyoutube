            ...
            <div v-else class="list-group">
              <a v-for="i in 15" :key="i" href="#" class="list-group-item list-group-item-action p-3 d-flex justify-content-between align-items-center" @click.prevent="showBlockedItems=true">
                ...
                <div class="popularity">
                  <font-awesome-icon v-for="x in 10" :key="x" icon="star" />
                </div>
              </a>
            </div>
          </v-card-text>
...
<script lang="ts">
import { BlockedChannel } from '@/types/authentication';
...
export default defineComponent({
  ...
  methods: {
    ...
    async handleAddBlockedKeyWord () {
      ...
      this.blockedKeyword = { word: '', duration: 'Forever' }
    },
    /**
     * 
     */
    async handleRemoveBlockedKeyword (index: number) {
